...
router.post("/register", async (req, res) => {
  if (req.body.password.length < 5 || req.body.password.length > 16) {
    return res.status(400).json({
      sucess: false,
      message: "password length should be in range of 5 to 16 charecter",
    });
  }
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ sucess: false, message: "user with this emil dosent exist" });
    }
    ...
    if (req.body.forAdmin) {
      if (!user.isAdmin) {
        return res
          .status(401)
          .json({ sucess: false, message: "wrong credentials" });
      }
    }
...
router.post("/forgotpass", async (req, res) => {
  ...
  if (!email)
    return res
      .status(400)
      .json({ sucess: false, message: "please provide a email" });
  ...
  try {
    ...
    try {
      ...
      return res
        .status(401)
        .json({ sucess: false, message: "Failed to send email" });
    }
...
router.post("/resetpassword/:resetToken", async (req, res) => {
  ...
  try {
    ...
    const oldPassHAsh = cryptoJS.AES.decrypt(
      user.password,
      process.env.CRYPTOJS_SECRET_KEY
    );
    ...
    if (oldPassword === newPassword) {
      return res
        .status(401)
        .json({ error: "you can not add your current password" });
    }
